...
public class CatalogoOscar {
    ...
    public static ArrayList<Filme> getFilmes() {
        ...
        // 21 — Terror/Ação: Pecadores (Oscar 2026 — 9 indicações, 4 vitórias)
        // Usando FilmeTerror como base (assumindo que aceita múltiplos gêneros no construtor ou descrição)
        FilmeTerror pecadores = new FilmeTerror(21, "Pecadores", 2025, 137,
                "Uma batalha após a outra no rastro de um mal antigo.",
                FaixaEtaria.MAIORES_16, 0.0, 9, 4, "vampiros e ação");
        pecadores.adicionarStreaming(Streaming.HBO_MAX);
        pecadores.adicionarStreaming(Streaming.AMAZON_PRIME); // Compra/Aluguel
        filmes.add(pecadores);

        // 22 — Ação/Drama: Uma batalha após a outra (Oscar 2026 — 9 indicações, 4 vitórias)
        FilmeAcao batalhaAposOutra = new FilmeAcao(22, "Uma batalha após a outra", 2025, 162,
                "Épico de Paul Thomas Anderson sobre sobrevivência e destino.",
                FaixaEtaria.MAIORES_16, 0.0, 9, 4, 9);
        batalhaAposOutra.adicionarStreaming(Streaming.HBO_MAX);
        filmes.add(batalhaAposOutra);

        // 23 — Terror/Ficção: Frankenstein (Oscar 2026 — 6 indicações, 3 vitórias)
        FilmeFiccaoCientifica frankenstein = new FilmeFiccaoCientifica(23, "Frankenstein", 2025, 150,
                "A releitura de Guillermo del Toro para o clássico de Mary Shelley.",
                FaixaEtaria.MAIORES_18, 0.0, 6, 3, "gótico");
        frankenstein.adicionarStreaming(Streaming.NETFLIX);
        filmes.add(frankenstein);

        // 24 — Comédia/Ficção: Bugonia (Oscar 2026 — 4 indicações)
        FilmeComedia bugonia = new FilmeComedia(24, "Bugonia", 2025, 119,
                "Dois homens obcecados por conspirações sequestram a CEO de uma grande empresa.",
                FaixaEtaria.MAIORES_18, 0.0, 4, 0, "humor ácido");
        bugonia.adicionarStreaming(Streaming.AMAZON_PRIME);
        filmes.add(bugonia);

        // 25 — Animação: Guerreiras do K-Pop (Oscar 2026 — 2 indicações, 2 vitórias)
        FilmeAnimacao kpopGuerreiras = new FilmeAnimacao(25, "Guerreiras do K-Pop", 2025, 96,
                "Um grupo de estrelas do K-Pop caça espíritos malignos entre shows.",
                FaixaEtaria.MAIORES_10, 0.0, 2, 2, "Estilizada");
        kpopGuerreiras.adicionarStreaming(Streaming.NETFLIX);
        filmes.add(kpopGuerreiras);

        // 26 — Animação: Zootopia 2 (Oscar 2026 — 1 indicação)
        FilmeAnimacao zootopia2 = new FilmeAnimacao(26, "Zootopia 2", 2025, 108,
                "Judy e Nick retornam para resolver um novo caso na metrópole animal.",
                FaixaEtaria.MAIORES_10, 0.0, 1, 0, "CGI");
        zootopia2.adicionarStreaming(Streaming.DISNEY_PLUS);
        filmes.add(zootopia2);

        // 27 — Drama: O Agente Secreto (Oscar 2026 — 4 indicações)
        // Representante brasileiro!
        FilmeDrama agenteSecreto = new FilmeDrama(27, "O Agente Secreto", 2025, 160,
                "Um homem foge para o Rio de Janeiro na década de 70, suspeito de ser um militante.",
                FaixaEtaria.MAIORES_16, 0.0, 4, 0, "político");
        agenteSecreto.adicionarStreaming(Streaming.NETFLIX);
        filmes.add(agenteSecreto);

        // 28 — Documentário: Mr. Nobody Against Putin (Oscar 2026 — 1 indicação, 1 vitória)
        FilmeDocumentario putinDoc = new FilmeDocumentario(28, "Mr. Nobody Against Putin", 2025, 90,
                "A trajetória de oposição política em um cenário de guerra e repressão.",
                FaixaEtaria.MAIORES_14, 0.0, 1, 1, "político");
        putinDoc.adicionarStreaming(Streaming.APPLE_TV);
        filmes.add(putinDoc);

        // 29 — Drama/Esporte: Marty Supreme (Oscar 2026 — 7 indicações)
        FilmeDrama martySupreme = new FilmeDrama(29, "Marty Supreme", 2025, 150,
                "Inspirado na vida de um mestre do ping-pong nos anos 50.",
                FaixaEtaria.MAIORES_16, 0.0, 7, 0, "biográfico");
        martySupreme.adicionarStreaming(Streaming.APPLE_TV); // Aluguel/Compra
        filmes.add(martySupreme);

        // 30 — Ação/Fantasia: Avatar: Fogo e Cinzas (Oscar 2026 — 2 indicações, 1 vitória)
        FilmeAcao avatar3 = new FilmeAcao(30, "Avatar: Fogo e Cinzas", 2025, 197,
                "Jake Sully e Neytiri enfrentam uma nova e agressiva tribo de Na'vi.",
                FaixaEtaria.MAIORES_14, 0.0, 2, 1, 10);
        avatar3.adicionarStreaming(Streaming.APPLE_TV); // Aluguel
        filmes.add(avatar3);
